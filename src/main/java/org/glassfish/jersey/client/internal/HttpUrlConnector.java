...
public class HttpUrlConnector implements Connector {
    ...
    private ClientResponse _apply(final ClientRequest request) throws IOException {
        ...
        if (request.resolveProperty(HttpUrlConnectorProvider.SET_METHOD_WORKAROUND, setMethodWorkaround)) {
            ...
            LOGGER.warning("Verb " + httpMethod +" not part of the Valid HTTP Methods yet - WorkAround used for " +
                    uc.getRequestMethod());
            setRequestMethodViaJreBugWorkaround(uc, httpMethod);
        } else {
            try {
                ...
                LOGGER.warning("Verb " + httpMethod +" not part of the Valid HTTP Methods yet - default verb is " +
                        uc.getRequestMethod());
            }
        }
